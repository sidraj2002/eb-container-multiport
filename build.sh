...
_ContainerLoopStop="false"
echo "Evaluating Dockerrun.aws.json for Port exposure.."
_ContainerLoopIterator=0

while [[ $_ContainerLoopStop != "true"  ]]
do

_ContainerLoopPortNum=$(cat Dockerrun.aws.json | jq -r .Ports[$_ContainerLoopIterator].ContainerPort)

if [[ $_ContainerLoopPortNum -eq null  ]]
then
_ContainerLoopStop="true"
echo "Container port evaluation complete."
else
_ContainerLoopPortAssign=$(cat Dockerrun.aws.json | jq -r .Ports[$_ContainerLoopIterator].ContainerPort)
_ContainerLoopPort="$_ContainerLoopPort $_ContainerLoopPortAssign"
fi
((_ContainerLoopIterator++))
done
echo "Container ports to expose $_ContainerLoopPort"
	...
	echo "EXPOSE $_ContainerLoopPort" >> Dockerfile
fi
